...
use embassy_net_driver::{PACKET_BUF_SIZE, PacketBuf, PacketBufAllocator};
...
impl TDes {
    ...
    #[cfg(feature = "ptp")]
    fn timestamp(&self) -> Option<embassy_net_driver::Timestamp> {
        (self.tdes3.get() & EMAC_TDES3_TTSS != 0)
            .then(|| embassy_net_driver::Timestamp::from_seconds_and_nanos(self.tdes1.get(), self.tdes0.get()))
    }
}
    ...
    pub(crate) fn poll_timestamp(&mut self) -> Option<embassy_net_driver::TxTimestamp> {
        loop {
                ...
                && let Some(id) = packet_id
            {
                ...
                break Some(embassy_net_driver::TxTimestamp { id, timestamp });
            }
        }
    }
...
impl<'a> RDesRing<'a> {
        ...
        for (i, desc) in descriptors.iter_mut().enumerate() {
            *desc = RDes::new();
            let buf = buffers[i].get_or_insert_with(|| {
                unwrap!(
                    allocator.try_alloc(),
                    "RX packet pool exhausted while filling the ethernet ring"
                )
            });
    ...
    pub(crate) fn receive(&mut self) -> Option<PacketBuf> {
        ...
        let Some(fresh) = self.allocator.try_alloc() else {
            warn!("RX packet pool exhausted, dropping received frame");
            self.pop_current();
            return None;
        };
