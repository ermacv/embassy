//! Raw sockets.
//!
//! A raw socket sends and receives whole packets, headers included: Ethernet
//! frames on one interface, or IP packets. The queue depth is set by the
//! `raw-rx-queue-count-N` feature of `xarxa`.
...
impl<'a> RawSocket<'a> {
    ...
    pub fn new(stack: Stack<'a>, ip_version: Option<IpVersion>, ip_protocol: Option<IpProtocol>) -> Self {
    ...
    /// Panics if the stack has no room for another raw socket. The limit is set
    /// by the `raw-socket-count-N` feature of `xarxa`.
    pub fn new_unbound(stack: Stack<'a>) -> Self {
    ...
    fn with<R>(&self, f: impl FnOnce(&mut raw::RawSocket<'_, 'static>) -> R) -> R {
        self.stack.with(|i| f(&mut i.stack.raw_socket(self.handle)))
    }

    fn with_mut<R>(&self, f: impl FnOnce(&mut raw::RawSocket<'_, 'static>) -> R) -> R {
