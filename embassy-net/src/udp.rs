//! UDP sockets.
//!
//! A UDP socket has no buffers of its own: received datagrams are queued as
//! owned packet buffers from the stack's pool, and sends go out synchronously.
//! The queue depth is set by the `udp-rx-queue-count-N` feature of `xarxa`.
...
use crate::{IpEndpoint, Stack, TryError};
...
impl<'a> UdpSocket<'a> {
    ...
    fn with<R>(&self, f: impl FnOnce(&mut udp::UdpSocket<'_, 'static>) -> R) -> R {
        self.stack.with(|i| f(&mut i.stack.udp_socket(self.handle)))
    }

    fn with_mut<R>(&self, f: impl FnOnce(&mut udp::UdpSocket<'_, 'static>) -> R) -> R {
